	...
	if (cplaylist->queue_track_refs) {
		...
		while (list) {
			...
			if (!gtk_tree_path_compare(path, lpath)) {
				...
				cplaylist->queue_track_refs = g_slist_remove_all(cplaylist->queue_track_refs, ref);
			}
			if (gtk_tree_model_get_iter(model, &iter, lpath)){
...
GtkTreePath* get_first_random_track(struct con_win *cwin)
{
	...
	} while (cplaylist->tracks_curr_playlist > 1 && (path == NULL));
...
static GtkTreePath* get_next_random_track(struct con_win *cwin)
{
	...
	do {
		...
		path = current_playlist_nth_track(rnd, cwin);
		 ...
		 (cplaylist->tracks_curr_playlist > 1));
...
static GtkTreePath* get_next_random_ref_track(struct con_win *cwin)
{
	...
	i = g_list_find(cplaylist->rand_track_refs, cwin->cstate->curr_rand_ref);
	if (i) {
		j = g_list_next(i);
		if (j)
...
static void clear_rand_track_refs(struct con_win *cwin)
{
	...
	if (cplaylist->rand_track_refs) {
		...
		g_list_free(cplaylist->rand_track_refs);
...
static void clear_queue_track_refs(struct con_win *cwin)
{
	...
	if (cplaylist->queue_track_refs) {
		...
		g_slist_free(cplaylist->queue_track_refs);
...
void jump_to_path_on_current_playlist (GtkTreePath *path, struct con_win *cwin)
{
	...
	if (path) {
		...
		gtk_tree_view_get_visible_rect(GTK_TREE_VIEW(cplaylist->wplaylist), &vrect);
		gtk_tree_view_get_cell_area(GTK_TREE_VIEW(cplaylist->wplaylist), path, NULL, &crect);

		gtk_tree_view_convert_widget_to_tree_coords(GTK_TREE_VIEW(cplaylist->wplaylist), crect.x, crect.y, &cx, &cy);
...
void select_last_path_of_current_playlist(struct con_win *cwin)
{
	...
	ref = g_strdup_printf("%d", cplaylist->tracks_curr_playlist - 1);
...
GtkTreePath* current_playlist_get_prev(struct con_win *cwin)
{
	...
	if (seq_first && cwin->cpref->repeat)
		path = current_playlist_nth_track((cplaylist->tracks_curr_playlist-1),
						  cwin);
...
void remove_from_playlist(GtkAction *action, struct con_win *cwin)
{
	...
	if (list) {
		...
		gtk_tree_view_get_cursor(GTK_TREE_VIEW(cplaylist->wplaylist), &next, NULL);
		do {
		...
		while(next != NULL);
		gtk_tree_view_set_cursor (GTK_TREE_VIEW(cplaylist->wplaylist), next, NULL, FALSE);
...
void clear_current_playlist(GtkAction *action, struct con_win *cwin)
{
	...
	cplaylist->tracks_curr_playlist = 0;
				       ...
				       struct con_win *cwin)
{
	...
	if(GTK_WIDGET(current_playlist) == GTK_WIDGET(cwin->cplaylist0->wplaylist)) {
		cwin->cstate->ref_to_playlist = cwin->playlist_used = 0;
				  ...
				  struct con_win *cwin)
{
	...
	list = gtk_tree_view_get_columns(GTK_TREE_VIEW(cplaylist->wplaylist));
					 ...
					 struct con_win *cwin)
{
	...
	case 1:
		...
		break;
	case 3:
		if ( cplaylist->tracks_curr_playlist == 0) {
				   ...
				   struct con_win *cwin)
{
	...
	is_row = gtk_tree_view_get_dest_row_at_pos(GTK_TREE_VIEW(cplaylist->wplaylist),
						x, y,
						&dest_path,
						&pos);

	gtk_tree_view_get_visible_rect(GTK_TREE_VIEW(cplaylist->wplaylist), &vrect);
	gtk_tree_view_get_cell_area(GTK_TREE_VIEW(cplaylist->wplaylist), dest_path, NULL, &crect);

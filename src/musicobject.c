...
AND LOCATION.id = \"%d\";", location_id, location_id);
	if (!exec_sqlite_query(query, cwin->cdbase, &result)) {
		...
		i = result.no_columns;
		mobj = g_object_new (PRAGHA_TYPE_MUSICOBJECT,
		                     "file", sanitize_string_from_sqlite3(result.resultp[i+12]),
		                     "file-type", atoi(result.resultp[i]),
		                     "title", sanitize_string_from_sqlite3(result.resultp[i+11]),
		                     "artist", sanitize_string_from_sqlite3(result.resultp[i+10]),
		                     "album", sanitize_string_from_sqlite3(result.resultp[i+9]),
		                     "genre", sanitize_string_from_sqlite3(result.resultp[i+8]),
		                     "comment", sanitize_string_from_sqlite3(result.resultp[i+5]),
		                     "year", atoi(result.resultp[i+6]),
		                     "track_no", atoi(result.resultp[i+7]),

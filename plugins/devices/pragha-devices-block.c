...
pragha_block_device_mount_finish (GVolume *volume, GAsyncResult *result, PraghaDevicesPlugin *plugin)
{
	...
	if (g_volume_mount_finish (volume, result, &error)) {
		...
		if (mount != NULL) {
			pragha_block_device_mounted (pragha_device_get_udev_device(plugin), mount, &error);
...
pragha_devices_moutable_added (PraghaDevicesPlugin *plugin, GUdevDevice *device)
{
	...
	pragha_gudev_set_hook_device (plugin, device, NULL, busnum, devnum);
	 ...
	 */
	g_timeout_add_seconds(5, pragha_block_device_mount, plugin);
}